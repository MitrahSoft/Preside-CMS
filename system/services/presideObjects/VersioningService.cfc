...
// PUBLIC API METHODS
	...
	public numeric function saveVersionForUpdate(
		...
		var existingRecords = poService.selectData( objectName = arguments.objectName, id=( arguments.id ?: javacast("null","") ), filter=arguments.filter, filterParams=arguments.filterParams, allowDraftVersions=true, fromVersionTable=true );
	...
	private array function _getVersionedManyToManyFieldsForObject( required string objectName ) {
		...
		for( var propertyName in properties ) {
			if ( poService.isManyToManyProperty( arguments.objectName, propertyName ) ) {
				...
				var versionedByDefault = !len( trim( relatedVia ) ) || poService.objectIsAutoGenerated( relatedVia );
